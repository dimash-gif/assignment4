// --- START OF FINAL CORRECTED FILE my-project-master/src/texture_mapping.cpp ---
...
int main() {
    ...
    GLFWwindow* window = glfwCreateWindow(windowWidth, windowHeight, "Part 3.1 - Texture Mapped Bezier Patch", NULL, NULL);
    ...
    patchShader = makeProgram("shaders/texture_130.vert", "shaders/texture_130.frag");
    patchTexture = loadTexture("assets/container.jpg");
    
    glGenVertexArrays(1, &patchVAO);
    glGenBuffers(1, &patchVBO);
    updatePatchGeometry();

    cout << "--- Bezier Patch Texture Mapping ---\n" << "Controls: W/S/A/D to orbit camera, Z/X to zoom.\n";

    while (!glfwWindowShouldClose(window)) {
        if (glfwGetKey(window, GLFW_KEY_A) == GLFW_PRESS) camAngle -= 1.0f;
        if (glfwGetKey(window, GLFW_KEY_D) == GLFW_PRESS) camAngle += 1.0f;
        if (glfwGetKey(window, GLFW_KEY_W) == GLFW_PRESS) camPitch = glm::min(89.0f, camPitch + 1.0f);
        if (glfwGetKey(window, GLFW_KEY_S) == GLFW_PRESS) camPitch = glm::max(-89.0f, camPitch - 1.0f);
        if (glfwGetKey(window, GLFW_KEY_Z) == GLFW_PRESS) camDist = glm::max(0.5f, camDist - 0.1f);
        if (glfwGetKey(window, GLFW_KEY_X) == GLFW_PRESS) camDist += 0.1f;
        ...
        glUniform3f(glGetUniformLocation(patchShader, "light_position"), 5.0f, 5.0f, 5.0f);
        glUniform1f(glGetUniformLocation(patchShader, "material_shininess"), 32.0f);
        
        glActiveTexture(GL_TEXTURE0);
        glBindTexture(GL_TEXTURE_2D, patchTexture);
        glUniform1i(glGetUniformLocation(patchShader, "diffuseMap"), 0);
...
vec3 evaluateBezierPatch(float u, float v) {
    vec3 u_curve_points[4]; // Was 'vec3 u_curve_points;' - needs to be an array
    for (int i = 0; i < 4; ++i) {
        u_curve_points[i] = evaluateBezierCurve(controlPoints[i * 4 + 0], controlPoints[i * 4 + 1], controlPoints[i * 4 + 2], controlPoints[i * 4 + 3], u);
    }
    return evaluateBezierCurve(u_curve_points[0], u_curve_points[1], u_curve_points[2], u_curve_points[3], v);
}
...
void updatePatchGeometry() {
    ...
    for (int i = 0; i < tessellationLevel; ++i) {
        for (int j = 0; j < tessellationLevel; ++j) {
            ...
            vec3 p00 = evaluateBezierPatch(u0, v0), p10 = evaluateBezierPatch(u1, v0);
            vec3 p01 = evaluateBezierPatch(u0, v1), p11 = evaluateBezierPatch(u1, v1);
...
GLuint loadTexture(char const * path) {
    ...
    if (data) {
        ...
        if (nrComponents == 1) format = GL_RED; else if (nrComponents == 4) format = GL_RGBA;
    ...
    } else { cout << "Texture failed to load at path: " << path << endl; }
    stbi_image_free(data);
    return textureID;
}

void framebuffer_size_callback(GLFWwindow* /*window*/, int width, int height) { glViewport(0, 0, width, height); windowWidth=width; windowHeight=height; }
